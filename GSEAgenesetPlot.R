...
GSEAgenesetPlot <- function(FC_list, 
                            geneset_db = "H", 
                            geneset_name, 
                            custom_db = NULL,
  ...
  if (!is.null(custom_db)) {
    ...
    temp_gmt <- msigdbr::msigdbr(species = "Homo sapiens", collection = geneset_db) %>%
      split(.$gs_name) %>%
      lapply(function(x) x$gene_symbol)
  }
  ...
  # If a gene is highlighted, add annotation
  ...
  m <- fgsea_res$stats %>%
    ...
    theme_classic() + 
    geom_vline(xintercept = median(fgsea_res$stats$rank), linetype = 2, color = "grey25") +
    xlab("Rank") + ylab("Statistic")
  ...
  n <- fgsea_res$stats %>%
    ...
    geom_text(aes(x = xpos[[10]], y = 1, label = right_label),
              color = right_label_col, fontface = "bold", size = 5, check_overlap = TRUE) +
    xlab("Rank")
